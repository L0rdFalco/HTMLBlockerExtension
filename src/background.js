...
async function isBlockingOn() {
  ...
  try {
    ...
    let blockStatus = await chrome.tabs.sendMessage(mTab.id, { action: "getStatus" })
...
async function toggleBlocker(mTabID) {

  try {
    await chrome.tabs.sendMessage(mTabID, { action: "toggle", status: allowed })
