...
async function blockerClicked() {
  if (!active) {
    //for ffx
    active = await chrome.permissions.request({ origins: ["*://*/*"] })
  ...
  */
  ...
  let res = await chrome.tabs.sendMessage(mTab.id, { action: "toggle", status: active }).catch(() => {
  ...
  if (!res) {// meaning that blocker isn't active  on said page
    ...
    try {
      ...
      await chrome.tabs.sendMessage(mTab.id, { action: "toggle", status: active })
    } catch (error) {
